...
if ($_SESSION ['status'] == 0) {
	...
	if ($_GET ['lang'] == cpp) {
		...
		$_SESSION ['language'] = 2;
		$t1=$mysqli->query ( "UPDATE `teams` SET `language` = 2, `status` = 1, `stage`='1b' WHERE `teamname` = '{$_SESSION['teamName']}'" );
		if($t1=== true){
...
//		GetInstructions ( 'c' );
		...
		$_SESSION ['language'] = 1;
		$t1=$mysqli->query ( "UPDATE `teams` SET `language` = 1, `status` = 1, `stage`='1a' WHERE `teamname` = '{$_SESSION['teamName']}'" );
		if($t1=== true){
